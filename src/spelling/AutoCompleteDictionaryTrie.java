...
public class AutoCompleteDictionaryTrie implements  Dictionary, AutoComplete {
    ...
    public boolean addWord(String word) {
        ...
        int ct = 0;
        while(ct < characters.length){
            Character ch = characters[ct];
            Set children = trieNode.getValidNextCharacters();
            if(!children.contains(ch)) {
                trieNode.insert(ch);
                if(ct == characters.length-1){
                	trieNode.getChild(ch).setEndsWord(true); 
                	size++; 
                	return true;
                }
            }
            trieNode = trieNode.getChild(ch);
            if (trieNode.getText().equals(word) && !isWord(trieNode.toString())){trieNode.setEndsWord(true); size++; return true;}
